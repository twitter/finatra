...
import com.twitter.finatra.streams.transformer.{IteratorImplicits, SamplingUtils}
...
class SamplingServerTopologyFeatureTest extends TopologyFeatureTest with IteratorImplicits{
  ...
  private val tweetIdToImpressingUserId =
    topologyTester.topic(SamplingServer.tweetToImpressingUserTopic, ScalaSerdes.Long, ScalaSerdes.Long)
  ...
  override def beforeEach(): Unit = {
    ...
    countStore = topologyTester.driver.getKeyValueStore[Long, Long](SamplingUtils.getNumCountsStoreName(SamplingServer.sampleName))
    sampleStore = topologyTester.driver.getKeyValueStore[IndexedSampleKey[Long], Long](SamplingUtils.getSampleStoreName(SamplingServer.sampleName))
  }
  ...
  private def assertSampleSize(tweetId: Int, expectedSize: Int): Unit = {
    val range = sampleStore.range(IndexedSampleKey(tweetId, 0), IndexedSampleKey(tweetId, Int.MaxValue))
