...
private object AbstractScalaObjectMapperTest {
...
abstract class AbstractScalaObjectMapperTest extends Test {
  ...
  test("deserialization#parse SimpleClassWithInjection fails") {
    // with no Injector: SimpleClassWithInjection has a @TestInjectableValue annotation but since
    // the mapper is not configured with a non-null injector, null is returned and the case class
    // fails to parse since the field is required and no value can be located from injection
    // even though we provide the field in JSON. If it has an injectable annotation, it MUST
    // be populated from injection or from a field default value.
    // with an Injector: SimpleClassWithInjection has a @TestInjectableValue annotation for which
    // there is no Jackson InjectableValues that supports the annotation, thus even though we
    // provide the field in JSON, nothing can inject the value, thus this fails.
    val json =
      """{
      |  "hello" : "Mom"
      |}""".stripMargin
    ...
    intercept[CaseClassMappingException] {
      parse[SimpleClassWithInjection](json)
    }
  }
  ...
  private def deserialize[T: Manifest](mapper: JacksonObjectMapper, value: String): T =
