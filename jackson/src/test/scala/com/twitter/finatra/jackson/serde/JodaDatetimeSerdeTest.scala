...
class JodaDatetimeSerdeTest extends Test {
  ...
  test("deserialize long") {
    val mapper = new JacksonObjectMapper()
    mapper.configure(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS, true)
    mapper.registerModule(BaseSerdeModule)
  ...
  test("deserialize text with ScalaObjectMapper with BaseSerdeModule") {
    val mapper = ScalaObjectMapper()
    mapper.registerModule(new JodaModule)
    mapper.registerModule(BaseSerdeModule)
  ...
  test("roundtrip text with ScalaObjectMapper with BaseSerdeModule") {
    val mapper = ScalaObjectMapper()
    mapper.registerModule(new JodaModule)
    mapper.registerModule(BaseSerdeModule)
    val json = mapper.writeValueAsString(myClass1)
