...
package object caseclass {
  ...
  private[finatra] def resolveSubType(
  ...
  private[finatra] def isNotAssignableFrom(clazz: Class[_], other: Class[_]): Boolean = {
    clazz != null && !isAssignableFrom(other, clazz)
  }

  /** boxes both sides for comparison -- no way to turn the from into the to type */
  private[finatra] def isAssignableFrom(fromType: Class[_], toType: Class[_]): Boolean =
    Types.wrapperType(toType).isAssignableFrom(Types.wrapperType(fromType))

  /** Create a new [[BeanProperty]] */
  private[finatra] def newBeanProperty(
    valueId: Object,
    context: DeserializationContext,
    javaType: JavaType,
    ...
    val mutator: AnnotatedMember = optionalJavaType match {
      case Some(optJavaType) =>
        newAnnotatedParameter(context, annotatedParameter, jacksonAnnotations, optJavaType, index)
      case _ =>
        newAnnotatedParameter(context, annotatedParameter, jacksonAnnotations, javaType, index)
    }
  ...
  /** Create an [[AnnotatedMember]] for use in a [[BeanProperty]] */
  private[finatra] def newAnnotatedParameter(
    context: DeserializationContext,
