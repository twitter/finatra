...
settings. This is true for building all of Finatra *except* when in master and
building the `finatra/examples`. In [master][master-branch], the examples are
defined such that they are able to be built with your locally installed
[sbt][sbt] since they are defined with their own `build.sbt` files and use
released Twitter OSS dependencies.
