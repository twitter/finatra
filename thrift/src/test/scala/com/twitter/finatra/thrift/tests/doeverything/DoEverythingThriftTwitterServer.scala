...
class DoEverythingThriftTwitterServer(clientRequestHeaderKey: String)
  ...
  private val service = DoEverything.ReqRepServicePerEndpoint(
    uppercase = Service.mk[Request[Uppercase.Args], Response[Uppercase.SuccessType]] {
      request: Request[Uppercase.Args] =>
        logRequestHeaders(request.headers, assertClientRequestHeaderKey = true)
        Future.value(
          Response(
            Map("com.twitter.doeverything.thriftscala.doeverything.uppercase" -> Seq(
              Buf.Utf8("response"))),
            request.args.msg.asInstanceOf[String].toUpperCase))
    },
    echo = Service.mk[Request[Echo.Args], Response[Echo.SuccessType]] {
      request: Request[Echo.Args] =>
        ...
        Future.value(
          Response(
            Map("com.twitter.doeverything.thriftscala.doeverything.echo" -> Seq(
              Buf.Utf8("response"))),
            request.args.msg))
    },
    echo2 = Service.mk[Request[Echo2.Args], Response[Echo2.SuccessType]] {
      request: Request[Echo2.Args] =>
        logRequestHeaders(request.headers)
        Future.value(Response(request.args.msg))
    },
    magicNum = Service.mk[Request[MagicNum.Args], Response[MagicNum.SuccessType]] {
      request: Request[MagicNum.Args] =>
        logRequestHeaders(request.headers)
        Future.value(
          Response(
            Map("com.twitter.doeverything.thriftscala.doeverything.magicNum" -> Seq(
              Buf.Utf8("response"))),
            "42"))
    },
    moreThanTwentyTwoArgs =
      Service.mk[Request[MoreThanTwentyTwoArgs.Args], Response[MoreThanTwentyTwoArgs.SuccessType]] {
        request: Request[MoreThanTwentyTwoArgs.Args] =>
          logRequestHeaders(request.headers)
  ...
  def main(): Unit = {
    thriftServer = ThriftMux.server
      .serveIface(thriftPortFlag(), service.filtered(serverSideLoggingFilter).toThriftService)
