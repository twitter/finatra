  ...
  def register(clazz: Class[_], service: ClassTag[_], method: Method): Unit = {
    ...
    if (method.getParameterTypes.nonEmpty) {
      registry.put(
        ...
        method.getParameterTypes.map { paramClass =>
          if (paramClass.getTypeParameters.nonEmpty) {
            paramClass.getName + paramClass.getTypeParameters.map(_.getName).mkString("[", ",", "]")
    ...
    registry.put(
      ...
      if (method.getReturnType.getTypeParameters.nonEmpty) {
        method.getReturnType.getName +
          method.getReturnType.getTypeParameters.map(_.getBounds.head.getTypeName).mkString("[", ",", "]")
