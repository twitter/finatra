...
@Singleton
class ClientIdWhitelistFilter @Inject()(whitelist: Set[ClientId], statsReceiver: StatsReceiver)
  ...
  private val unknownClientIdException =
    Future.exception(new UnknownClientIdError("unknown client id"))
  private val noClientIdException =
    Future.exception(new NoClientIdError("The request did not contain a Thrift client id"))
