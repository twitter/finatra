...
class InheritanceThriftMethodBuilderClientModuleFeatureTest extends FeatureTest with HttpTest {

  private val httpClientId: String = "http-server"

  private val thriftServer = new EmbeddedThriftServer(
    twitterServer = new TestThriftServer(new ServiceBThriftService(httpClientId).toThriftService),
    disableTestLogging = true
  )

  val server = new EmbeddedHttpServer(
    twitterServer = new ServiceBHttpServer,
    args = Seq(
      s"-thrift.clientId=$httpClientId",
      resolverMap("serviceB-thrift-service" -> thriftServer.thriftHostAndPort)
    )
  )
  ...
  test("echo") {
    ...
    server.inMemoryStats.counters
      .get("clnt/serviceB-thrift-client/ServiceA/echo/failures") should be(None)
    /* assert MethodBuilder stats exist */
    server.inMemoryStats.stats
      .get("clnt/serviceB-thrift-client/echo/logical/request_latency_ms") should not be None
  ...
  test("ping") {
    ...
    server.inMemoryStats.counters
      .get("clnt/serviceB-thrift-client/ServiceB/ping/failures") should be(None)
    /* assert MethodBuilder stats exist */
    server.inMemoryStats.stats
      .get("clnt/serviceB-thrift-client/ping/logical/request_latency_ms") should not be None
