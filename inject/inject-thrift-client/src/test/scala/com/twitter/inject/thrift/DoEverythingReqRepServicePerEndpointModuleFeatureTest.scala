...
import com.twitter.inject.thrift.DoEverythingReqRepServicePerEndpointModuleFeatureTest._
import com.twitter.inject.thrift.integration.reqrepserviceperendpoint.{ReqRepServicePerEndpointHttpController, GreeterReqRepServicePerEndpointModule, GreeterThriftService}
import com.twitter.inject.thrift.integration.{TestHttpServer, TestThriftServer}
import com.twitter.util.Duration
import com.twitter.util.tunable.Tunable

object DoEverythingReqRepServicePerEndpointModuleFeatureTest {
  case class HelloHeaders(empty: Boolean)
  case class HelloResponse(value: String, headers: HelloHeaders)
}

class DoEverythingReqRepServicePerEndpointModuleFeatureTest
  extends FeatureTest
  with HttpTest {
  ...
  override val server = new EmbeddedHttpServer(
    twitterServer = new TestHttpServer[ReqRepServicePerEndpointHttpController](
      "rrspe-server",
      new GreeterReqRepServicePerEndpointModule(requestHeaderKey, perRequestTimeoutTunable)),
    args = Seq(
