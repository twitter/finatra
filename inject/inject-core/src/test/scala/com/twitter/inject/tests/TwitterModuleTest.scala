...
class TwitterModuleTest extends Test {
  ...
  test("provide helpful error when bound to self") {
    ...
    assert(sources.contains(this.getClass.getName))
  }
  ...
  test("multibinder#bind [T, Ann]") {
    val module = new TwitterModule {
      override def configure(): Unit = {
        val multi = bindMultiple[String, Prod]
        multi.addBinding.toInstance("A")
        multi.addBinding.toInstance("B")
      }
    }
    validate(Injector(Guice.createInjector(module)).instance[immutable.Set[String], Prod], "A", "B")
  }
  ...
  test("multibinder#bind parameterized [T, Ann]") {
    val module = new TwitterModule {
      override def configure(): Unit = {
        val mbStrings = bindMultiple[W[String], Prod]
        mbStrings.addBinding.toInstance(W("A"))
        val mbInts = bindMultiple[W[Int], Prod]
        mbInts.addBinding.toInstance(W(1))
      }
    }

    val injector = Injector(Guice.createInjector(module))
    validate(injector.instance[immutable.Set[W[String]], Prod], W("A"))
    validate(injector.instance[immutable.Set[W[Int]], Prod], W(1))
  }
  ...
  test("optionalbinder#bind parameterized [T, Ann]") {
    val module = new TwitterModule {
      override def configure(): Unit = {
        val sOpt = bindOption[W[String], Prod]
        sOpt.setDefault.toInstance(W("A"))
        val nOpt = bindOption[W[Int], Prod]
        nOpt.setDefault.toInstance(W(1))
      }
    }

    validateWithAnn[W[String], Prod](module, expected = W("A"))
    validateWithAnn[W[Int], Prod](module, expected = W(1))
  }
