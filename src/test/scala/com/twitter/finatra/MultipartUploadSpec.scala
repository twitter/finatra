...
class MultipartUploadSpec extends FlatSpecHelper {
  ...
  "Multi part uploads with text and file fields" should "work on POST" in {
    ...
    ***/
    ...
    response.getHeader("X-Test-Method") should equal("POST")
  }

  it should "work on PUT" in {
    ...
    ***/
    ...
    val s = getClass.getResourceAsStream("/put.bytes")
    val b = IOUtils.toByteArray(s)
    val r = FinagleRequest.decodeBytes(b)
    send(r)
    ...
    response.getHeader("X-Filename") should equal("Some(dealwithit.gif)")
    response.getHeader("X-Type-Text") should equal("text")
    response.getHeader("X-Test-Method") should equal("PUT")
  }
