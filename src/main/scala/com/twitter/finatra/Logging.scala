...
trait Logging {
  ...
  def stdoutHandlerFactory = () => new ConsoleHandler

  def fileHandlerFactory = {
    FileHandler(
  ...
  def initLogger() {
    val handler: HandlerFactory = if (Config.getBool(FinatraParams.logToStdout)) stdoutHandlerFactory else fileHandlerFactory
