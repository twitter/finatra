...
class ValidationMessageResolver {
  ...
  private def load: Properties = {
    ...
    loadPropertiesFromSystemProperty(properties)
    properties
  }
  ...
  private def loadPropertiesFromSystemProperty(properties: Properties) {
    sys.props.get("finatra.validation.properties.path") map { path =>
